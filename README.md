...
```cmd
scripts\Build-Installer.cmd 0.0.16 win-x64
```

Build the smaller framework-dependent installer without the .NET runtime:

```cmd
scripts\Build-Installer.cmd 0.0.16 win-x64 no-runtime
```

This publishes a self-contained `win-x64` release with `PublishReadyToRun=true`, then builds:

```text
artifacts\installer\SideDock-0.0.16-win-x64.msi
```

The no-runtime variant builds:

```text
artifacts\installer\SideDock-0.0.16-win-x64-no-runtime.msi
```
...
```cmd
msiexec /i artifacts\installer\SideDock-0.0.16-win-x64.msi
```
...
Create and publish a GitHub Release with a tag like `v0.0.16`. The `Release MSI` workflow will build and attach both MSI release assets from that tag:

```text
SideDock-0.0.16-win-x64.msi
SideDock-0.0.16-win-x64-no-runtime.msi
```
